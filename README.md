...
src/
  ...
  providers/    Interfaces first, implementations behind them
    types.ts      WeatherProvider · TrafficProvider · MountainProvider · PlacesProvider
    demo/         The demo bundle (scenario, weather, traffic, mountain, places)
...
Coverage spans: snow-clock physics (track-out rate, terrain spread, wind,
control delays, late openings), prime-window selection, scoring (factor
interactions, weight configuration, imputation, extremes, penalties), morning
departure optimisation (rush-hour curves, prime capture, drive ceilings,
preference response), return optimisation (afternoon walls, stay-vs-go,
waiting past last chair), the recommendation layer (ranking, explanation,
determinism, "more snow ≠ better day"), LATER (confidence decay, best-bet
discounting), demo-provider determinism and differentiation, provider-failure
handling, and the UI end-to-end including honest empty states and
...
- ~87 kB gzipped, no web fonts, no external requests
...
| `MountainProvider` | Lift/terrain status and snow reports; crowd signal (visitation, parking, lift-scan data) |
...
requirements, parking, and multi-day trips.
