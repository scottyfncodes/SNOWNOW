...
export { CotripRoadProvider, LiveTrafficProvider, NwsAlertsProvider, OpenMeteoWeatherProvider };
...
/**
 * The live bundle. Mountain operations, ticket pricing and places have no
 * reliable public machine-readable source for arbitrary Colorado resorts —
 * rather than scrape fragile HTML, this deliberately keeps those three on the
 * demo/static implementations and says so (README → "Live Data Status").
 * Weather and alerts are genuinely live; traffic is live only when a server
 * base URL is configured; road conditions are live-but-unverified and opt-in.
 */
export function createLiveRegistry(options: LiveRegistryOptions = {}): ProviderRegistry {
  ...
  return {
    ...
    mountain: new DemoMountainProvider(),
    ...
    // "Any slot still demo" — true here because mountain/pricing/places
    // always are, and traffic/roads may be depending on configuration.
    usingDemoData: true,
